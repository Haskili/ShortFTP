...
./server $portNum $password $serverOptions > ./server.out &

# Generate a random file for transfer process testing
touch ./tempTransferTestFile
openssl rand -base64 300 > ./tempTransferTestFile 2> /dev/null
...
./client $defaultAddr $portNum $password > ./client.out << INPUT
y
tempTransferTestFile
INPUT

echo "Process complete, checking results in client.out for verification..."

# Verify that the client got a good response from server for SHA1 check on their downloaded file
getValid=`sed -n '/Good response from server for SHA1 verification/p' client.out`
if [ "$getValid" == "C: Good response from server for SHA1 verification, confirmed that our file is valid. Moving on..." ];
	then echo "Success, transfer was completed and file was verified with a SHA1 check"
else
	echo "Failure. Please check the client.out for more information"
fi
